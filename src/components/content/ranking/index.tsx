...
export const Ranking = () => {
  ...
  const userWithPoints = data
    ? data.users
        ...
        .map((user) => ({
          ...user,
          points:
            calculateAllPoints(data.matches, user.matches) +
            calculateAllWinnerPoints(data.winners, user.winners),
        }))
        // @ts-expect-error
        .sort(function (a, b) {
  ...
  const firstMatchDate: string = data
    ? data.matches[0]
      ? data.matches[0].date
      : ""
    : "";

  const avaiableMatches =
    data &&
    data.matches &&
    data.matches
      // @ts-expect-error
  ...
  const getPoints = (match: Match, user: any) => {
    const betA =
      user.matches && user.matches[match.id] && user.matches[match.id].betA;
    const betB =
      user.matches && user.matches[match.id] && user.matches[match.id].betB;
  ...
  const bestUserIndexes = userWithPoints.reduce(
    (indexes: any, user: any, index: number) => {
        ...
        <Accordion alwaysOpen>
          {
            ...
            userWithPoints.map((user, index) => (
              <Accordion.Item eventKey={user.nick} key={user.nick}>
                <Accordion.Header>
                  <Badge
                    bg={`${bestUserIndexes.includes(index) && user.points > 0 ? "warning" : "primary"}`}
                    pill
                  >
                    {user.points}
                  </Badge>
                  <div className="mx-2">
                    {bestUserIndexes.includes(index) && user.points > 0 ? (
                      <span>👑</span>
                    ) : null}
                  </div>
                    ...
                    avaiableMatches.map((match) => (
                      ...
                      >
                        ...
                        <div className="col-2 d-flex justify-content-center">
                          <Badge bg="info">
                            {(user.matches &&
                              user.matches[match.id] &&
                              user.matches[match.id].betA) ||
                              "X"}
                            :
                            {(user.matches &&
                              user.matches[match.id] &&
                              user.matches[match.id].betB) ||
                              "X"}
                          </Badge>
                        </div>
                        <div className="col-2">
                      ...
                      <div className="row">
                        {user.winners && (
                          <div className="col-12">
                            ...
                            {data.winners &&
                              (data.winners.winner != null
                                ? data.winners.winner === user.winners.winner
                                  ? " ✅"
                                  : " ❌"
                                : null)}
                          </div>
                        ...
                        {user.winners &&
                          Object.keys(user.winners)
                            .filter((key) => key !== "winner")
                            .map((key) => (
                              <div className="col-6 text-truncate">
                                {key}: <b>{user.winners[key]}</b>
                                {data.winners &&
                                  (data.winners[key] != null
                                    ? data.winners[key] === user.winners[key]
                                      ? " ✅"
                                      : " ❌"
                                    : null)}
                              </div>
