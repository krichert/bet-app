...
export const MatchesAcordion = ({
  ...
  const [formData, setFormData] = useState<any>(
    user && user.matches ? user.matches : {},
  );
  ...
  const areChangedAlreadyStartedMatches = () => {
    const now = moment();
    const alreadyStartedMatchIndexes = matches
      .filter((match) => moment(now).isAfter(moment.utc(match.date)))
      .map((m) => m.id);
    const changedMatchIndexes = Object.keys(formData)
      .map((key) => ({ ...formData[key], id: key }))
      .filter((match) => {
        const existing = user.matches ? user.matches[match.id] : undefined;
        const existingBetA = existing ? existing.betA : undefined;
        const existingBetB = existing ? existing.betB : undefined;

        return match.betA !== existingBetA || match.betB !== existingBetB;
      })
      .map((m) => m.id);

    return changedMatchIndexes.some((i) =>
      alreadyStartedMatchIndexes.includes(i),
    );
  };
  ...
  const sortedMatches = matches
    ...
    .sort((a, b) =>
      moment
        .utc(a.date)
        .tz("Europe/Warsaw")
        .diff(moment.utc(b.date).tz("Europe/Warsaw")),
    );

  const getPoints = (match: Match) => {
    const betA =
      userMatches && userMatches[match.id] && userMatches[match.id].betA;
    const betB =
      userMatches && userMatches[match.id] && userMatches[match.id].betB;
    ...
    <Accordion.Item eventKey={day}>
      <Accordion.Header>
        <Badge
          className="mx-3"
          bg={
            calculateAllPoints(matches, userMatches) > 0
              ? "primary"
              : "secondary"
          }
        >
          {calculateAllPoints(matches, userMatches)}
        </Badge>{" "}
        Mecze {day}
      </Accordion.Header>
      <Accordion.Body>
        {sortedMatches.map((match) => (
          <div key={match.id} className="row mb-3">
            ...
            <div className="row d-flex align-items-center">
              <div className="col-3 d-flex justify-content-end px-3">
                {match.teamA}
              </div>
              <div className="col-6 p-0">
                <div className="row d-flex align-items-center">
                  <div className="col-5 d-flex justify-content-center p-0">
                    {isEditMode ? (
                      <Form.Control
                        disabled={isInputDisabled(match.date)}
                        name="betA"
                        value={
                          formData &&
                          formData[match.id] &&
                          formData[match.id].betA
                        }
                        onChange={(e) => handleChange(e, match.id)}
                        type="number"
                      />
                    ) : (
                      <p className="m-0">
