...
export const WinnerAcordion = ({ isBlocked }: { isBlocked: boolean }) => {
  ...
  const [formData, setFormData] = useState<any>(
    user && user.winners ? user.winners : {},
  );
  ...
  return (
    <Accordion.Item eventKey="winners">
      <Accordion.Header>
        <Badge className="mx-3" bg="secondary">
          {calculateAllWinnerPoints(winners, userWinners)}
        </Badge>
        Zwycięzcy
      </Accordion.Header>
      <Accordion.Body>
          ...
          <Form.Group>
            ...
            {isEditMode ? (
              ...
              >
                ...
                {Object.keys(teams)
                  // @ts-expect-error
                  .reduce<string[]>((acc, curr) => [...acc, ...teams[curr]], [])
                  .map((team) => {
              ...
              <p className="text-muted">
                ...
                {winners &&
                  userWinners &&
                  // @ts-expect-error
                  (winners.winner != null && userWinners.winner != null
                    ? // @ts-expect-error
                      winners.winner === userWinners.winner
                      ? " ✅"
                      : " ❌"
                    : null)}
              </p>
              ...
              <Form.Group>
                ...
                {isEditMode ? (
                  ...
                  <p className="text-muted">
                    ...
                    {winners &&
                      userWinners &&
                      (winners[el] != null && userWinners[el] != null
                        ? winners[el] === userWinners[el]
                          ? " ✅"
                          : " ❌"
                        : null)}
                  </p>
